...
func TestWorkflowsCompileV1_ValidationErrorRoundTrip(t *testing.T) {
	issuesBytes := readConformanceWorkflowV1FixtureBytes(t, "workflow_v1_invalid_condition.issues.json")
	...
	_, err = client.Workflows.CompileV1(context.Background(), WorkflowSpecV1{Kind: WorkflowKindV1})
	if err == nil {
		t.Fatalf("expected validation error")
	}
	verr, ok := err.(WorkflowValidationError)
...
func TestWorkflowsCompileV1_SuccessRoundTrip(t *testing.T) {
	planJSON := readConformanceWorkflowV1FixtureBytes(t, "workflow_v1_router.plan.json")
	hash := PlanHash("0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef")
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(WorkflowsCompileResponseV1{
			PlanJSON: planJSON,
			PlanHash: hash,
		})
	}))
	...
	resp, err := client.Workflows.CompileV1(context.Background(), WorkflowSpecV1{Kind: WorkflowKindV1})
	if err != nil {
		t.Fatalf("compile: %v", err)
	}
	if resp.PlanHash != hash {
